...
public class OrderStateFromBillingConsumer {
    private final OrderService orderService;

    public static final String ERROR_MESSAGE ="Invalid message";

    @KafkaListener(groupId = "${kafka.consumer.group-id}",
            topics = {"${kafka.topics.order-payment-accepted}", "${kafka.topics.order-payment-declined}"},
            containerFactory = "orderStateFromBillingKafkaListenerContainerFactory",
            batch = "false")
    public void consume(ConsumerRecord<String, String> consumerRecord) throws ValidationException {
        if (consumerRecord != null && StringUtil.isNotEmpty(consumerRecord.value())) {
            try {
                orderService.processOrderStateFromBillingKafkaCommand(consumerRecord.value());
            } catch (Exception e) {
